...
class TestTokenizer(u.TestCase):
    ...
    def testComment(self):
        ...
        output = good(input[5:], None, Token("comment", " abc", first))
        self.assertEqual(run(tokenp, input), output)

    def testDataOpen(self):
        input = np.addLineCol("data_hello 123abc")
        output = good(input[10:], None, Token("dataopen", "hello", first))
        self.assertEqual(run(tokenp, input), output)
    
    def testSaveOpen(self):
        input = np.addLineCol("save_hello 123abc")
        output = good(input[10:], None, Token("saveopen", "hello", first))
        self.assertEqual(run(tokenp, input), output)
    
    def testSaveClose(self):
        input = np.addLineCol("save_ hello 123abc")
        output = good(input[5:], None, Token("saveclose", "save_", first))
        self.assertEqual(run(tokenp, input), output)
    
    def testLoop(self):
        input = np.addLineCol("loop_ hello 123abc")
        output = good(input[5:], None, Token("loop", "loop_", first))
        self.assertEqual(run(tokenp, input), output)
    
    def testStop(self):
        input = np.addLineCol("stop_ hello 123abc")
        output = good(input[5:], None, Token("stop", "stop_", first))
        self.assertEqual(run(tokenp, input), output)
    ...
    def testValue(self):
        ...
        for (str, consumed, value, message) in cases:
            ...
            output = good(input[consumed:], None, Token("value", value, first))
            print repr(run(tokenp, input)) + "\n" + repr(output)
            self.assertEqual(run(tokenp, input), output, message)
    
    def testWhitespace(self):
        input = np.addLineCol(" \t\t\t  blar")
        output = good(input[6:], None, Token("whitespace", " \t\t\t  ", first))
        self.assertEqual(run(tokenp, input), output)
    
    def testNewlines(self):
        input = np.addLineCol("\n\n\r\f\f\r \t\t\t  blar")
        output = good(input[6:], None, Token("newlines", "\n\n\r\f\f\r", first))
        self.assertEqual(run(tokenp, input), output)
    
    def testIdentifier(self):
        input = np.addLineCol("_abcd123 \t\t\t  blar")
        output = good(input[8:], None, Token("identifier", "abcd123", first))
        self.assertEqual(run(tokenp, input), output)
    
    def testScanner(self):
        self.assertEqual(run(tok.scanner, np.addLineCol('"abc')), 
        ...
        self.assertEqual(run(tok.scanner, np.addLineCol('# abc \n123')),
            m.pure({'rest': [], 'state': None, 'result': [Token('comment', ' abc ', {'line': 1, 'column': 1}),
                                                          Token('newlines', '\n', {'line': 1, 'column': 7}),
